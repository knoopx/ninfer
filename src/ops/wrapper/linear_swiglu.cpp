...
std::size_t linear_swiglu_workspace_capacity_bytes(QType qtype, std::int32_t gate_up_rows,
    ...
    if (qtype == QType::TERNARY_PQ2_0 && gate_up_rows == 34816 && input_rows == 5120) {
        return detail::ternary_linear_swiglu_workspace_capacity_bytes(policy, min_tokens,
                                                                      max_tokens);
    }
...
void linear_swiglu(const Tensor& x, const Weight& gate_up_weight, Tensor& out, LinearPolicy policy,
    ...
    const bool ternary_weight = large_shape && gate_up_weight.qtype == QType::TERNARY_PQ2_0;
    const bool fp8_weight   = large_shape && gate_up_weight.qtype == QType::FP8_E4M3FN_ROW_BF16;
    if (!q4_weight && !q8_weight && !nvfp4_weight && !ternary_weight && !fp8_weight) {
    ...
    if (ternary_weight) {
        (void)detail::validate_ternary_weight_metadata(gate_up_weight, "ternary linear_swiglu");
        detail::ternary_linear_swiglu_dispatch(x, gate_up_weight, out, policy, ws, stream);
        return;
    }
