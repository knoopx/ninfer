...
// Multi-model (no-resident) model object: the webui-shaped entry for one configured model,
// carrying the status marker (loaded|unloaded) the webui's discovery panel reads. No engine
// metadata (no-resident: nothing is loaded at discovery time).
Json make_model_object_json(const ModelConfig& model, std::int64_t created,
...
// Multi-model (no-resident) discovery: list every configured model, marking the loaded one
// (the webui's discovery panel reads status.value per entry).
std::string make_models_list(const std::vector<ModelConfig>& models, std::int64_t created,
    ...
    for (const auto& model : models) {
        data.push_back(make_model_object_json(model, created, max_model_len, model.id == loaded_id));
    }
    const Json payload = {{"object", "list"}, {"data", data}};
    return payload.dump();
}

std::string make_model_object(const ModelConfig& model, std::int64_t created,
                              std::uint32_t max_model_len, bool loaded) {
    return make_model_object_json(model, created, max_model_len, loaded).dump();
}
