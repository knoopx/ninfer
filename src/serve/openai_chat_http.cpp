...
void HttpServer::handle_chat_completions(const httplib::Request& req, httplib::Response& res) {
    ...
    // alias -> 404. The Grant keeps the granted backend (Engine) alive for the whole request;
    // the streaming path moves it into the stream. Request concurrency admission (429
    // server_overloaded) is handled by the engine layer (prepare/run), not the router.
