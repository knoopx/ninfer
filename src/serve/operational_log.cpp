...
void OperationalLog::model_preset(const EngineModelBackend& backend, const ModelConfig& model,
    ...
    // Identity + admission: request-concurrency admission (the engine layer's bounded FIFO
    // ingress, owned by GenerationService) is governed by the shared --max-concurrency
    // (ServeOptions.max_concurrency); the router no longer admits-limits requests.
