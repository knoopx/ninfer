...
void HttpServer::handle_model_load(const httplib::Request& req, httplib::Response& res) const {
    ...
    // (openai_chat_http.cpp): an unknown id/alias -> 404. Request concurrency admission (429
    // server_overloaded) is handled by the engine layer, not the router. In the single-GPU,
    // one-resident model this load evicts the current resident (a sequential swap, no preemption);
    // extra_args are ignored (NInfer model config is fixed at registration).
