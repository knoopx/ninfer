...
void HttpServer::handle_count_tokens(const httplib::Request& req, httplib::Response& res) {
    ...
    // Route (supersedes model validation): unknown model -> 404 model-not-found. Request concurrency
    // admission (429 server_overloaded) is handled by the engine layer, not the router. The Grant
    // keeps the backend alive for the (short) count call.
...
void HttpServer::handle_messages(const httplib::Request& req, httplib::Response& res) {
    ...
    // Route through the in-process router: unknown model -> 404 model-not-found. Request
    // concurrency admission (429 server_overloaded) is handled by the engine layer, not the
    // router. The Grant keeps the granted backend alive for the whole request; the streaming path
    // moves it into the stream.
    ModelRouter::Grant grant;
    try {
        grant = router_->route(request.model);
    } catch (const std::out_of_range& exception) {
