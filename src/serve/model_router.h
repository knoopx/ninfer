...
// llama-swap FIFO scheduler (internal/router/scheduler/fifo.go). Bounded model-swap join queue:
// requests for a model that is mid-swap are held in arrival order and drained front-to-back when
// the swap completes (the queue is bounded; drop/reject when full). Unlike the llama-swap
// sketch's "A B C A B C -> A A B B C C" cross-model reordering, this router has no cross-model
// ordering guarantee: route() drains the queue after each swap (drain()) and joiners re-route in
// wake order, so only bounded, arrival-ordered ingress with drop/reject when full holds.
//
// Request concurrency admission (bounded FIFO ingress with 429 server_overloaded / 503
// request_queue_timeout) is owned by the engine layer (GenerationService); the router does NOT
// admit-limit requests. The scheduler only bounds the model-swap join queue and tracks in-flight
// counts for the TTL pin (an in-flight request pins the backend against eviction).
//
// Signatures are the decision-tree helpers that mirror fifo.go so the router reads faithfully.
// Two deliberate, documented deviations from the sketch:
//   - enqueue() returns bool (true=accepted, false=dropped-when-full) so the bounded rejection
//     is observable (the sketch says void but "drop/reject when full" must be reportable).
...
class FifoScheduler {
public:
    // max_queue_size bounds the pending queue (drop/reject when full). Default 1024.
    explicit FifoScheduler(std::size_t max_queue_size = 1024);
...
public:
    ...
    // Constructs the router and starts the 1s TTL ticker thread. The router starts with NO loaded
    // backend (no-resident, on-demand-load); the factory is NOT called at construction -- the
    // first request for a model triggers the load path from the empty state (do_swap from a null
    // resident + the readiness gate). The factory must not throw when it is later called for a
    // load; a load failure surfaces via route()'s "model is not ready" rejection.
    ...
    // the Option-B doSwap. Unknown id/alias throws std::out_of_range ("model '<id>' not found";
    // the handler maps it to 404). A failed load or readiness timeout throws std::exception (the
    // handler maps it to 503). Request concurrency admission (bounded FIFO ingress, 429
    // server_overloaded / 503 request_queue_timeout) is owned by the engine layer
    // (GenerationService); the router does NOT reject on a concurrency limit. The returned Grant
    // keeps the backend alive for the request and releases the in-flight reservation on
    // destruction.
