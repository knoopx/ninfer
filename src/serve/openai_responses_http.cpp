...
void HttpServer::handle_responses(const httplib::Request& req, httplib::Response& res) {
    ...
    // The Grant keeps the granted backend alive for the whole request; the streaming path moves
    // it into the stream. Request concurrency admission (429 server_overloaded) is handled by the
    // engine layer (prepare/run), not the router.
...
void HttpServer::handle_response_input_tokens(const httplib::Request& req, httplib::Response& res) {
    try {
        ...
        // Route (supersedes validate_openai_model): unknown model -> 404. Request concurrency
        // admission (429 server_overloaded) is handled by the engine layer, not the router.
        ModelRouter::Grant grant;
        try {
