...
using Geometry = TernaryN14336K5120;  // K-driven constants only (N is unused).
...
int capacity_admits() {
    ...
    for (auto policy : {ops::LinearPolicy::A16Only, ops::LinearPolicy::AllowA8,
        ...
        const std::size_t capacity =
            ops::attn_input_proj_workspace_capacity_bytes(QType::TERNARY_PQ2_0, 14336, 5120, policy,
                                                          1, 8);
        if (capacity != 0) {
            std::cerr << "capacity: [14336,5120] policy " << static_cast<int>(policy)
...
int capacity_rejects() {
    ...
    const std::pair<std::int32_t, std::int32_t> shapes[] = {{14336, 5121}, {16384, 5120}, {1, 1}};
    for (auto [n, k] : shapes) {
        failures += expect_invalid_argument(
            ("capacity: unregistered [" + std::to_string(n) + ',' + std::to_string(k) +
             "] must throw").c_str(),
            [n, k]() {
                (void)ops::attn_input_proj_workspace_capacity_bytes(QType::TERNARY_PQ2_0, n, k,
    ...
    for (auto [min_tokens, max_tokens] : {std::pair{1, 0}, std::pair{5, 3}}) {
        failures += expect_invalid_argument(
            ...
            [min_tokens, max_tokens]() {
                (void)ops::attn_input_proj_workspace_capacity_bytes(QType::TERNARY_PQ2_0, 14336, 5120,
...
int weight_validation() {
    ...
    // The valid (14336, 5120) fixture must validate, with view.signs pointing just past the
    // rotation header.
    {
        auto f = make_ternary_fixture(14336, 5120);
        bool ok = true;
        try {
            ...
            std::cerr << "weight: valid (14336,5120) fixture rejected: " << error.what() << '\n';
        }
        if (!ok) ++failures;
    }
    ...
    {
        auto f = make_ternary_fixture(14336, 129);  // K must be a multiple of the 128 group size.
...
int main() {
    try {
        ...
        std::cout << (failures == 0 ? "OK" : "FAIL") << " TERNARY_PQ2_0 AttnInputProj (host)\n";
        return failures;
    } catch (const std::exception& error) {
        std::cerr << "TERNARY_PQ2_0 AttnInputProj (host): " << error.what() << '\n';
        return 1;
    }
}
