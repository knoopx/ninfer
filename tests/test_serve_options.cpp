...
ServeOptions parse(std::vector<std::string> arguments) {
    ...
    config_args.reserve(arguments.size() + 2);
    for (const auto& argument : arguments) {
        if (argument == "model.ninfer") { continue; }
        config_args.push_back(argument);
        if (argument == "ninfer-serve") {
            config_args.push_back("--config");
            config_args.push_back(std::string(NINFER_SOURCE_DIR) +
                                  "/tests/fixtures/serve-config.json");
        }
    }
    std::vector<char*> argv;
    argv.reserve(config_args.size());
    for (std::string& argument : config_args) { argv.push_back(argument.data()); }
...
int main() {
    ...
    failures +=
        check(configured.preserve_thinking == true,
