...
//      reloads it.
//   6. Concurrency admission  - the router does NOT reject on a concurrency limit; a third
//      concurrent route() call for the resident model is admitted (no exception).
...
ns::ServeOptions make_options(const Config& config, int max_concurrency = 10) {
    ...
    // ServeOptions.max_concurrency is the engine's compact-decode-batch count; the router does
    // NOT use it for admission (that is the engine layer's job). Keep the option for the engine.
...
void test_4_fifo_ordering() {
    std::printf("[test] 4. FIFO ordering (arrival order, drain front-to-back) + bounded\n");
    ...
    // In-flight accounting (TTL pin): begin/release track per-model counts.
    FifoScheduler accounting(16);
    CHECK(accounting.in_flight("m") == 0, "no in-flight initially");
    accounting.begin_in_flight("m");
    CHECK(accounting.in_flight("m") == 1, "one in-flight after begin");
    accounting.begin_in_flight("m");
    CHECK(accounting.in_flight("m") == 2, "two in-flight after second begin");
    accounting.release_in_flight("m");
    CHECK(accounting.in_flight("m") == 1, "release decrements the in-flight count");
    CHECK(accounting.in_flight_total() == 1, "total in-flight reflects the decrement");
    accounting.release_in_flight("m");
    CHECK(accounting.in_flight("m") == 0, "fully released: back to zero");
}
...
void test_6_concurrency_admission() {
    std::printf("[test] 6. concurrency admission: the router does NOT reject on a concurrency limit\n");
    FakeState state;
    Config config = make_config(/*other_ttl*/ 0);
    ModelRouter router(make_options(config), {}, make_factory(&state));

    // Hold multiple in-flight grants for the same model: all are admitted (no rejection).
    ...
    CHECK(router.in_flight() == 2, "two in-flight after the second grant");
    // A third concurrent route() call is also admitted (the router does NOT limit concurrency).
    ModelRouter::Grant third = router.route("other"); // in-flight = 3
    CHECK(router.in_flight() == 3, "a third concurrent request is admitted (no concurrency limit)");
    CHECK(third.backend != nullptr, "the admitted third request holds a valid grant");
    // All three grants point to the same resident backend.
    CHECK(held[0].backend.get() == held[1].backend.get(), "grants share the same resident backend");
    CHECK(held[0].backend.get() == third.backend.get(), "all three grants share the same backend");
}
